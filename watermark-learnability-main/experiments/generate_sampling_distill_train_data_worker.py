...
def get_prompts(args, additional_num_skip: int = 0) -> Dict:
    ...
    dataset = load_dataset(args.dataset_name, args.dataset_config_name, split=args.dataset_split,
                           streaming=args.streaming)

    def encode(examples):
        prompt = tokenizer(
            examples[args.data_field], truncation=True, padding=True, max_length=args.prompt_length,
            return_tensors="pt",
        ).to(device)
